...
help() {
    ...
    echo "d     Specify the installation directory. Defaults to /usr/local/bin or the directory where aliae is installed."
    echo
}
...
validate_dependency() {
    if ! command -v $1 >/dev/null; then
        error "$1 is required to install aliae. Please install $1 and try again.\n"
    fi
}
...
set_install_directory() {
    ...
    # check if we have aliae installed, if so, use the executable directory
    # to install into and follow symlinks
    if command -v aliae >/dev/null; then
        posh_dir=$(command -v aliae)
        real_dir=$(realpath $posh_dir)
        install_dir=$(dirname $real_dir)
        info "aliae is already installed, updating existing installation in:"
        info "  ${install_dir}"
    else
...
install() {
    ...
    if [ "${good}" != "1" ]; then
        error "${arch} builds for ${platform} are not available for aliae"
    fi
