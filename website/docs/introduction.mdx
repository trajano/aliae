...
- Shell behavior and detection:
  ...
  - `aliae get shell --verbose` support for shell resolution trace output.
  - Windows Git Bash PATH delimiter handling fix.
- Template capabilities:
  - `homeFileExists` and `homeDirExists` helpers.
  - `progress` helper for OSC progress output and reset.
  - `.AliaeConfig` and `.AliaeConfigDir` template variables.
- Fork distribution and automation:
  - Fork-oriented installer/docs references for release artifacts.
  - In-repo Scoop manifest template with automated release-time generation.
