...
- Configuration capabilities:
  ...
  - Rendered `init` output cache (non-stateful scripts) keyed by shell/runtime and tracked env inputs.
...
## Shell strategy selection design pattern

The shell rendering layer uses a strategy selector:

- Selector chooses which shell strategy to use.
- `ShellFormatStrategy` encapsulates shell-specific rendering behavior.
- Composite composes section renderers, each implementing `Render()`, in a stable pipeline order.

### Selector and strategy class diagram

```mermaid
classDiagram
    class formatStrategy {
      +formatStrategy() ShellFormatStrategy
    }

    class ShellFormatStrategy {
      <<interface>>
    ...
    class zshFormatStrategy
    class bashFormatStrategy
    class pwshFormatStrategy
    class nuFormatStrategy
    class fishFormatStrategy
    class tcshFormatStrategy
    class xonshFormatStrategy
    class cmdFormatStrategy
    class noopFormatStrategy
    ...
    formatStrategy ..> ShellFormatStrategy : selects
    ShellFormatStrategy <|.. zshFormatStrategy
    ShellFormatStrategy <|.. bashFormatStrategy
    ShellFormatStrategy <|.. pwshFormatStrategy
    ShellFormatStrategy <|.. nuFormatStrategy
    ShellFormatStrategy <|.. fishFormatStrategy
    ShellFormatStrategy <|.. tcshFormatStrategy
    ShellFormatStrategy <|.. xonshFormatStrategy
    ShellFormatStrategy <|.. cmdFormatStrategy
    ShellFormatStrategy <|.. noopFormatStrategy

    Alias ..> ShellFormatStrategy : delegates formatting
    Env ..> ShellFormatStrategy : delegates formatting
    Path ..> ShellFormatStrategy : delegates formatting
    CDPath ..> ShellFormatStrategy : delegates formatting
    Link ..> ShellFormatStrategy : delegates formatting
    Echo ..> ShellFormatStrategy : delegates formatting
```
